// app.dev.mjs
// Dev-only server — identical UI to production, NO MongoDB, NO DataContainer.
// Run with: node app.dev.mjs  (defaults to port 3001)
...
// ---------------------------------------------------------------------------
// In-memory workout store (resets on restart — dev only)
// ---------------------------------------------------------------------------
const devSaved = [];
let   nextId   = 1;
...
const BASE_STYLES = `
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .nav-links { display: flex; gap: 16px; flex-wrap: wrap; color: var(--muted); font-size: 0.95rem; }
    .page-shell {
      max-width: 1100px; margin: 0 auto;
    ...
    .hero-tag {
      ...
      text-transform: uppercase; letter-spacing: 0.12em; font-size: 0.78rem; color: #d6e3ff;
    }
    .hero-intro h1 { margin: 14px 0 12px; font-size: clamp(2.4rem, 4vw, 3.2rem); }
    .hero-intro p  { color: var(--muted); line-height: 1.65; margin-bottom: 24px; }
    ...
    .hero-btn.primary {
      ...
      color: #0c1633; border: none;
      box-shadow: 0 16px 35px rgba(79,140,255,0.35);
    }
    .hero-btn.secondary { color: var(--text); background: transparent; }
    ...
    button, .primary-btn {
      ...
      font-size: 1rem; font-weight: 600; color: #0c1633;
      background: linear-gradient(135deg, #7fc4ff, var(--accent));
      cursor: pointer; box-shadow: 0 18px 35px rgba(79,140,255,0.35);
      transition: transform 0.2s ease;
    }
    button:hover, .primary-btn:hover { transform: translateY(-2px); }
    .plan-card {
      border: 1px solid var(--border); border-radius: 18px;
      padding: 18px; background: var(--panel-light);
      margin-bottom: 16px; box-shadow: 0 16px 40px rgba(0,0,0,0.35);
    }
    ...
    footer { text-align: center; color: var(--muted); padding: 24px 12px 40px; font-size: 0.85rem; }
...
function renderHistoryList() {
  ...
  return `
    <ul class="history-list">
      ${userState.completedPlans.map(e => `<li>${escapeHtml(e.summary)}</li>`).join('')}
    </ul>
  `;
}
...
app.get('/subscribe', (_req, res) => {
  ...
  const body = `
    <section class="panel">
      ...
      <p style="color:var(--muted);">No money moves here. Press the button so the planner unlocks.</p>
...
app.get('/planner', (_req, res) => {
  ...
  const body = `
    <section class="panel">
      ...
      <p style="color:var(--muted);">Choose one muscle group and the tools you actually have.</p>
      <form method="POST" action="/planner">
        <label>Your name (optional)<input type="text" name="name" placeholder="Sam Student"></label>
        <label>Muscle group<select name="muscle" required><option value="">Select...</option>${muscleOptions}</select></label>
        <label>Equipment on hand<select name="equipment" required><option value="">Select...</option>${equipmentOptions}</select></label>
...
app.post('/planner', (req, res) => {
  ...
  const planSummary = exercises.length
    ? exercises.map(ex => `
      ...
      `).join('')
    : '<p style="color:var(--muted);">No matching moves. Try different equipment.</p>';
  ...
  const body = `
    <section class="panel">
      ...
      <p style="color:var(--muted);">Run through each move for three sets of ten slow reps.</p>
      ${planSummary}
...
// ---------------------------------------------------------------------------
// Dev API — mirrors production /api/workouts shape, no MongoDB
// ---------------------------------------------------------------------------
...
app.post('/api/workouts', (req, res) => {
  const workout = { id: String(nextId++), ...req.body, createdAt: new Date().toISOString() };
...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', env: 'development', store: 'in-memory', workouts: devSaved.length });
});
...
app.listen(PORT, () => {
  ...
  console.log('[DEV] No MongoDB — workouts stored in memory, reset on restart.');
});
